...
MBTI_DIMENSION_OPTIONS = (
    ("MBTI_ei", "E / I", [("MBTI_A", "E"), ("MBTI_B", "I")]),
    ("MBTI_sn", "S / N", [("MBTI_D", "S"), ("MBTI_C", "N")]),
    ("MBTI_tf", "T / F", [("MBTI_E", "T"), ("MBTI_F", "F")]),
    ("MBTI_jp", "P / J", [("MBTI_H", "P"), ("MBTI_G", "J")]),
)
MBTI_QUESTION_ID = "MBTI_PROFILE"
MBTI_UNSURE_VALUE = "MBTI_I"
MBTI_QUESTION_TEXT = "如果你知道自己的 MBTI，也可以先选择更接近你的类型"
DISPLAY_ORDER = (MBTI_QUESTION_ID, "Q1", "Q2", "Q3", "Q4", "Q5", "Q6", "Q7", "Q8", "Q9", "Q10")
DISPLAY_NUMBER_MAP = {
    question_id: index for index, question_id in enumerate(DISPLAY_ORDER, start=1)
}
...
def collect_mbti_answer() -> str | list[str] | None:
    unsure = st.checkbox("我不确定", key="MBTI_unsure")
    if unsure:
        for key, _, _ in MBTI_DIMENSION_OPTIONS:
            if key in st.session_state:
                st.session_state[key] = None
        return MBTI_UNSURE_VALUE
...
def mbti_code_from_answers(answer_value: str | list[str] | None) -> str:
    if answer_value == MBTI_UNSURE_VALUE:
        return ""
...
def mbti_match_note(mbti_code: str, work_title: str, matched_tags: list[str]) -> str:
    ...
    return (
        f"作为 {mbti_code} 的你，{energy_clause[mbti_code[0]]}，"
        f"也会被{perception_clause[mbti_code[1]]}、{feeling_clause[mbti_code[2]]}、"
        f"{rhythm_clause[mbti_code[3]]}的作品打动。"
    )
...
def is_answer_complete(question_id: str) -> bool:
    if question_id == MBTI_QUESTION_ID:
...
def render_question_stage(
    ...
    for question_id in visible_ids:
        ...
        render_standard_question(
            question,
            question_number=DISPLAY_NUMBER_MAP.get(question.question_id, question.question_id),
        )

    return all(is_answer_complete(question_id) for question_id in visible_ids)


def render_mbti_question() -> bool:
    render_question_heading(
        MBTI_QUESTION_TEXT,
        question_number=DISPLAY_NUMBER_MAP[MBTI_QUESTION_ID],
    )
    st.caption("四组二选一；如果你暂时不想判断，也可以直接选“我不确定”。")
    st.session_state[MBTI_QUESTION_ID] = collect_mbti_answer()
    return is_answer_complete(MBTI_QUESTION_ID)
...
def main() -> None:
    legacy_q1 = st.session_state.get("Q1")
    if isinstance(legacy_q1, list) or legacy_q1 == "Q1_I":
        st.session_state.pop("Q1", None)
    for legacy_key in ("Q1_unsure", "Q1_ei", "Q1_sn", "Q1_tf", "Q1_jp"):
    ...
    all_visible_complete = is_answer_complete(MBTI_QUESTION_ID) and all(
    ...
    if show_results_stage:
        ...
        mbti_code = mbti_code_from_answers(st.session_state.get(MBTI_QUESTION_ID))
    ...
    stages_completed = render_mbti_question()
